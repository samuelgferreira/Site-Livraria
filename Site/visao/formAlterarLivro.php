...
<head>
	...
	<div class="container">
		<form action="livro.php?func=alterar" method = "post" enctype="multipart/form-data">
              <?php
                foreach ($usuario as $reg)
				{
						...
						echo "<p>E-mail:</p> <input type='text' name='autor' value='".$reg->getAutor()."'><br/>";
						echo"<p>Categoria: <br></p>";
						echo "<select name='categoria' size='1'>";
						echo "<option value='".$reg->getCategoria()."' >".$reg->getCategoria()."</option>";
							include_once "modelo/LivroDAO.class.php";
							//acessando a camada modelo
							$dao = new LivroDAO();
							$lista = $dao->ListarMenus();
							foreach ($lista as $r){	
								echo "<option value='".$r->getNome()."' >".$r->getNome()."</option>";
							}
						echo "</select>";
						echo "<br>
						<br>
						<p>Descrição:</p><div class='summernote container'>
						<textarea id='summernote' name='descricao' rows='10'>".$reg->getDescricao()."</textarea>
						</div>";
				}
               ?>
